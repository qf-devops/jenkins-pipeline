...
def getSaltToken(url, params) {
    ...
    authToken = sendHttpGetRequest("${url}/login", data, ['Accept': '*/*'])['return'][0]['token']
    return authToken
}
...
def createSaltConnection(url, credentialsId) {
    params = [
        ...
        "authToken": null,
        "creds": getPasswordCredentials(credentialsId)
    ]
...
def runSaltCommand(master, client, target, function, batch = null, args = null, kwargs = null) {
    ...
    return sendHttpPostRequest("${master.url}/", data, headers)
}

def getSaltPillar(master, target, pillar) {
    def out = runSaltCommand(master, 'local', target, 'pillar.get', none, [pillar.replace('.', ':')])
    return out
}
...
def enforceSaltState(master, target, state, output = false) {
    ...
    def out = runSaltCommand(master, 'local', target, 'state.sls', none, [run_states])
    try {
        checkSaltResult(out)
    } finally {
        if (output == true) {
...
def runSaltCmd(master, target, cmd) {
    return runSaltCommand(master, 'local', target, 'cmd.run', none, [cmd])
}
...
def generateSaltNodeKey(master, target, host, keysize = 4096) {
    ...
    return runSaltCommand(master, 'wheel', target, 'key.gen_accept', none, args, kwargs)
}
...
def generateSaltNodeMetadata(master, target, host, classes, parameters) {
    ...
    return runSaltCommand(master, 'local', target, 'reclass.node_create', none, args, kwargs)
}

def orchestrateSaltSystem(master, target, orchestrate) {
    return runSaltCommand(master, 'runner', target, 'state.orchestrate', none, [orchestrate])
}
