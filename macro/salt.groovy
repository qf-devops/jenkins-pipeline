...
def getSaltProcess(saltProcess) {
    ...
    def process_def = [
        ...
        'install_openstack_mcp_infra': [
            ...
            [tgt: '*', fun: 'saltutil.refresh_pillar'],
            // Sync all salt resources
            [tgt: '*', fun: 'saltutil.sync_all'],
            // Bootstrap all nodes
            [tgt: '*', fun: 'state.sls', arg: ['linux,openssh,salt.minion,ntp']],
            // Comment nameserver
...
def runSaltProcess(saltMaster, saltProcess) {

    tasks = getSaltProcess(saltProcess)

    for (i = 0; i <tasks.size(); i++) {
        task = tasks[i]
        echo("[Salt master ${saltMaster.url}] Task ${task}")
        if (task.containsKey('arg')) {
            runSaltCommand(saltMaster, 'local', ['expression': task.tgt, 'type': 'compound'], task.fun, task.arg)
        }
        else {
